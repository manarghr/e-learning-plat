        ...
        fetch("login.php", {
          ...
          .then((data) => {
            ...
            if (data.status === "success") {
              ...
              // Update header without page refresh (optional)
              updateHeaderAfterLogin(data.user_name)
              ...
              // If email doesn't exist, redirect to mentor form after a short delay
              if (data.message && data.message.includes("email")) {
                setTimeout(() => {
                  window.location.href = "mentor-form.html"
                }, 2000)
              }
            }
          })
...
function updateHeaderAfterLogin(userName) {
  const signInBtn = document.querySelector(".sign-in .btn-primary")
  if (signInBtn) {
    ...
    signInBtn.href = "logout.php"

    // Insert welcome message before the button
    signInBtn.parentNode.insertBefore(welcomeSpan, signInBtn)
  }
}
